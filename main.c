...
int main(void)
{
  ...
  while (1)
  {
    ...
    ADC=pollADC();//read the raw adc val
   char adc_buffer[16];
   sprintf(adc_buffer, "%lu",ADC);
   writeLCD(adc_buffer);//write to screen
...
void EXTI0_1_IRQHandler(void)
{
  ...
  long currentTick = HAL_GetTick();
	...
	if ((currentTick - lastTick) >= 500){
		if (delay_t == 500){ 
				delay_t = 1000; //1hz delay
		}
		else if (delay_t == 1000){
				delay_t = 500;   //2Hz delay
		}
	}
...
void writeLCD(char *char_in){
    delay(3000);
	lcd_command(CLEAR);
  lcd_putstring(char_in); //display on lcd
}
